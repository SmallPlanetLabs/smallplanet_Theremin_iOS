...
class VideoMixer {
	...
	func mix(videoPixelBuffer: CVPixelBuffer, depthPixelBuffer: CVPixelBuffer) -> CVPixelBuffer? {
		...
		guard let outputTexture = makeTextureFromCVPixelBuffer(pixelBuffer: outputPixelBuffer, textureFormat:.bgra8Unorm),
			let inputTexture0 = makeTextureFromCVPixelBuffer(pixelBuffer: videoPixelBuffer, textureFormat: .bgra8Unorm),
			let inputTexture1 = makeTextureFromCVPixelBuffer(pixelBuffer: depthPixelBuffer, textureFormat: .bgra8Unorm) else {
				return nil
		}
        

//        print("mixFactor = \(mixFactor)")
