...
class CameraViewController: UIViewController, AVCapturePhotoCaptureDelegate, AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureDepthDataOutputDelegate, AVCaptureDataOutputSynchronizerDelegate {
	...
	override func viewDidLoad() {
        ...
        //---
        print("viewDidLoad... \(NSStringFromCGRect(previewView.frame))");
		...
		*/
        ...
        var possibleLetters = ["w", "x", "y"]
        var array : [String] = [String]()
        
        for index in 0..<10000 {
            if (index > 5000 && index < 7000) {
                array.append("c")
            } else {
                let randomLetterIndex = Int(arc4random_uniform(UInt32(possibleLetters.count)))
                array.append(possibleLetters[randomLetterIndex])
            }
        }
        NSLog("array>count = \(array.count)")
        
//        DepthUtility.createImage(width: 100, height: 100, from: array) { image, errorMessage in
...
//                guard let cgImage = ghostImage?.cgImage else {
//                    NSLog("couldn't load ghost image")
//                    return
//                }
//                guard let pixelBuffer = DepthUtility.buffer(from: ghostImage!) else {
...
//                finalVideoPixelBuffer = mixedBuffer
	...
	func processDepth(depthData: AVDepthData) {
        ...
        if type == kCVPixelFormatType_DepthFloat16 ||
            type == kCVPixelFormatType_DisparityFloat16 {
            inputTextureFormat = .r16Float
        } else if type == kCVPixelFormatType_DepthFloat32 ||
            type == kCVPixelFormatType_DisparityFloat32 {
...
//        print("--------------------------");
        ...
        CVPixelBufferUnlockBaseAddress(depthData.depthDataMap, CVPixelBufferLockFlags.readOnly)
        ...
        if !videoDepthConverter.isPrepared {
			...
			*/
			...
			CMVideoFormatDescriptionCreateForImageBuffer(kCFAllocatorDefault, depth.depthDataMap, &depthFormatDescription)
...
//        }
        ...
        DepthUtility.createImage(depthPixelBuffer:depth.depthDataMap, pixelFormat: videoDepthConverter.inputTextureFormat, silhouetteThreshold: self.mixFactorSlider.value * 5, completionHandler: { (image, string) in
            ...
            DispatchQueue.main.async {
                ...
                // mixFactor.Value
                NSLog("threshold value = \(self.mixFactorSlider.value * 5)")
		...
		currentDepthPixelBuffer = depthData.depthDataMap
	}
...
//                print("depthDataWasDropped--0");
				let depthData = syncedDepthData.depthData
				processDepth(depthData: depthData)
            } else {
//                print("depthDataNOTdropped--1");
            }
		}
...
//                }
				...
				guard let mixedPixelBuffer = self.photoDepthMixer.mix(videoPixelBuffer: finalPixelBuffer, depthPixelBuffer: depthPixelBuffer) else {
